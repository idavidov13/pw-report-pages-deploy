...
- Name: Failing Test
- Location: /home/runner/work/Playwright-Framework/Playwright-Framework/tests/clientSite/failingTest.spec.ts:3:5
...
Expected: 3
Received: 2
    at /home/runner/work/Playwright-Framework/Playwright-Framework/tests/clientSite/failingTest.spec.ts:5:15
```
...
- text: May 13, 2025
- button " 0"
- link "ustulo cibus angulus Cunae universe vulnus compono virgo. Read more... cervus":
  - /url: /article/ustulo-cibus-angulus-26520
  - heading "ustulo cibus angulus" [level=1]
  - paragraph: Cunae universe vulnus compono virgo.
  - text: Read more...
  - list:
    - listitem: cervus
- link:
...
- text: Test YouTube Zoom Bondar Academy qa career QA Skills Value-Focused Git GitHub Live Sessions
- contentinfo:
  - link "conduit":
    - /url: /
...
```ts
  1 | import { test, expect } from '../../fixtures/pom/test-options';
  2 |
  3 | test('Failing Test', { tag: '@Regressions' }, async ({ homePage }) => {
  4 |     await homePage.navigateToHomePageUser();
> 5 |     expect(2).toEqual(3);
    |               ^ Error: expect(received).toEqual(expected) // deep equality
  6 | });
  7 |
```